...
func Test_EntryCSVCustomHeaders(t *testing.T) {
	entry := gmaps.Entry{
		...
		SocialLinks: []string{"https://facebook.com/joescafe", "https://instagram.com/joescafe"},
	}
	...
	require.Equal(t, expectedHeaders, entry.CsvHeaders())
	require.Equal(t, len(entry.CsvHeaders()), len(entry.CsvRow()))
	require.Equal(t, "Joe's Cafe", entry.CsvRow()[0])
	require.Equal(t, "Cafe", entry.CsvRow()[1])
	require.Equal(t, "https://maps.google.com/?cid=123", entry.CsvRow()[2])
	require.Equal(t, "123 Main St, Jackson, MS", entry.CsvRow()[3])
	require.Equal(t, "https://joes.com", entry.CsvRow()[4])
	require.Equal(t, "+1 555-1234", entry.CsvRow()[5])
	require.Equal(t, "joe@cafe.com", entry.CsvRow()[6])
	require.Equal(t, "https://facebook.com/joescafe, https://instagram.com/joescafe", entry.CsvRow()[7])
}
